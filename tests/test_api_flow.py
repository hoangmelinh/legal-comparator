...
class ApiFlowTests(unittest.TestCase):
    ...
    def test_chat_stats_answers_without_ollama(self):
        app_module.compare_results["job_chat_stats"] = {
            "compare_job_id": "job_chat_stats",
            "changes": [
                {
                    "id": "chg_1",
                    "article": "Điều 1",
                    "status": "MODIFIED",
                    "summary": "Điều khoản thanh toán thay đổi.",
                    "evidence": {"citation_a": "A", "citation_b": "B"},
                },
                {
                    "id": "chg_2",
                    "article": "Điều 2",
                    "status": "ADDED",
                    "summary": "Thêm điều khoản bảo mật.",
                    "evidence": {"citation_a": "", "citation_b": "B"},
                },
            ],
        }
        ...
        with patch.object(
            ...
            response = self.client.post(
                ...
                json={
                    "workflow_id": "wf_chat",
                    "compare_job_id": "job_chat_stats",
                    "message": "Có bao nhiêu điều khoản bị sửa?",
                    "history": [],
                },
            )

        self.assertEqual(response.status_code, 200)
        self.assertIn("Có 1 thay đổi loại sửa đổi", response.text)
        self.assertIn("CHG-001", response.text)

    def test_chat_analysis_response_is_cached_by_question_and_job(self):
        app_module.compare_results["job_chat_cache"] = {
            "compare_job_id": "job_chat_cache",
            "changes": [
                {
                    "id": "chg_1",
                    "article": "Điều 5",
                    "status": "MODIFIED",
                    "summary": "Thời hạn thanh toán thay đổi từ 10 ngày sang 15 ngày.",
                    "evidence": {
                        "citation_a": "Thanh toán trong 10 ngày.",
                        "citation_b": "Thanh toán trong 15 ngày.",
                    },
                    "text_a": "Thanh toán trong 10 ngày.",
                    "text_b": "Thanh toán trong 15 ngày.",
                }
            ],
        }
        ...
        with patch.object(
            app_module,
            "stream_chat_ollama",
            return_value=iter(["Thanh toán thay đổi từ 10 sang 15 ngày. [CHG-001]"]),
        ) as mocked_llm:
            first = self.client.post(
                ...
                json={
                    "workflow_id": "wf_chat",
                    "compare_job_id": "job_chat_cache",
                    "message": "Điều khoản thanh toán thay đổi thế nào?",
                    "history": [],
                },
            )
        self.assertEqual(first.status_code, 200)
        self.assertEqual(mocked_llm.call_count, 1)
        self.assertIn("CHG-001", first.text)

        with patch.object(
            app_module,
            "stream_chat_ollama",
            side_effect=AssertionError("Cached answer should not call Ollama"),
        ):
        ...
        self.assertIn("CHG-001", second.text)
